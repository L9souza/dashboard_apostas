...
# Formatar delta com BRL e seta para baixo
if variacao_banca < 0:
    delta_text = f"🔻 R$ {abs(variacao_banca):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    delta_color = "inverse"
else:
    delta_text = f"🔺 R$ {variacao_banca:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    delta_color = "normal"
